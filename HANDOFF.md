...
- `node_modules` e `dist/` **non più tracciati** (commit `b2dc825`, ora in `.gitignore`). Prima erano committati (da prima del .gitignore): i ~5k file di `node_modules` facevano crashare `gh-pages` su Windows con `spawn ENAMETOOLONG` — checkout di `main` nella cache + `git rm` con ogni path come arg separato sforava il limite ~32k della command line. Niente più churn da `node_modules/.package-lock.json`. Se ENAMETOOLONG ritorna: svuotare la cache con `rm -rf node_modules/.cache/gh-pages`.
